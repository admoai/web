...
const ResponsiveImage = forwardRef(
  (
    ...
    return (
      <div
        className={cn(className, position, 'w-full block overflow-hidden')}
        ref={ref}
        style={
          originalDimensions
            ? { maxWidth: `${image?.asset?.metadata?.dimensions?.width}px` }
            : containerStyle
        }
      >
        {!originalDimensions && !resolvedMobileAspect && (
          <>
            <div
              className='hidden medium:block aspect-holder'
              style={{
                paddingTop: fullHeight ? '100vh' : `${100 / resolvedAspect}%`
              }}
            />
            <div
              className='block medium:hidden aspect-holder'
              style={{
                paddingTop: fullHeight ? '100svh' : `${100 / resolvedAspect}%`
              }}
            />
          </>
        )}
        ...
        <Picture
          className={cn(
            imageClassName,
            !preload && 'opacity-0',
            !originalDimensions && 'absolute w-full h-full inset-0',
